...
public sealed class ApplicationCompositionTests
{
  ...
  [Fact]
  public void ProductionComposition_WiresWorkspaceTransferCoordinator()
  {
    ...
    Assert.Contains("new SqliteWorkspaceTransferStore(", source);
    Assert.Contains("new ExportWorkspace(", source);
    Assert.Contains("new InspectWorkspaceRestore(", source);
    Assert.Contains("new RestoreWorkspace(", source);
    Assert.Contains("new WorkspaceTransferCoordinator(", source);
    Assert.Contains("workspaceTransferCoordinator", source);
  }
  ...
  [Fact]
  public void RuntimeInTestModeStillDetectsPortableMarkerBeforeUsingManagedRootOverride()
  {
    using var executableDirectory = new TemporaryDirectory();
    using var localApplicationData = new TemporaryDirectory();
    using var managed = new TemporaryDirectory();
    File.WriteAllText(Path.Combine(executableDirectory.Path, "portable.flag"), "");
    var previousTestMode = Environment.GetEnvironmentVariable("TWW3_COMPANION_TEST_MODE");
    var previousManagedRoot = Environment.GetEnvironmentVariable("TWW3_COMPANION_TEST_MANAGED_ROOT");
    ...
    try
    {
      ...
      {
        Assert.NotNull(runtime);
        Assert.Equal(ApplicationMode.Portable, runtime.ManagedPaths.Mode);
        Assert.Equal(Path.Combine(executableDirectory.Path, "Data"), runtime.ManagedPaths.RootDirectory);
        Assert.NotEqual(managed.Path, runtime.ManagedPaths.RootDirectory);
      }
    }
    finally
